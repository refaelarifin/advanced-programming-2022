...
class Loading():
    ...
    def __animated_loading():
        chars = "/—\|"
        for char in chars:
            sys.stdout.write('\r' + Color.print_colored('loading...',
                             utils=["bold"]) + Color.print_colored(char, color_fg=[10, 120, 10]))
            sleep(.1)
            sys.stdout.flush()

    @classmethod
    def loading(cls, process_fnc, args):
        s = threading.Thread(
            name='process', target=process_fnc, args=args, daemon=True)
        s.start()
